...
class IonList(object):
    ...
    @property
    def E(self):
        return isotope.ufunc_E(self._ions)

    @property
    def idx(self):
        return isotope.ufunc_idx(self._ions)
...
class AbuDump(AbuData):
    ...
    def idx(self):
        ...
        mean molecular weight per electron
        """
        return isotope.ufunc_idx(self.ions)
    ...
    *** Add a way to hold non-normalized values?
    ...
    def write_bg(self,
        ...
        for i,a in zip(iso, abu):
            f.write("{:s} {:s} {:14.8E} {:s}\n".format(card_mix,mixture,a,i.Kepler()))
        if write_net:
            f.write(card_cmt + '\n')
    ...
    def _in1d(ar1, ar2):
        ...
        """
        ar1 = isotope.ufunc_idx(ar1)
        ar2 = isotope.ufunc_idx(ar2)
    ...
    def __call__(self, *args):
        ...
        if len(args) == 1:
            ...
            if isinstance(arg, str):
                ...
                ref = [arg]
        assert len(val) > 0
        val = isotope.ion(list(val))
        ref = isotope.ion(list(ref))
