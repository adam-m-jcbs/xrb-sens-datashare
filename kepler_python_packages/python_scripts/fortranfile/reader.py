...
class DataInputBuffer(object):
    ...
    def warn_eor(self):
        """Warn if current position is not end of record.
    ...
    ############################################################
    # peek routines - fundamental loading
    ############################################################
    ...
    def peek_n(self, dim=None, dtype=None, *, offset=0, order='F', output='C', tail=None, pad=None, padval=np.nan):
        ...
        KW only
          ...
          padval - valu to use for padding
        """
        if type(dim) == type:
            dtype, dim = dim, dtype
        if dtype is None:
        ...
        if output is None:
            output = order
        value = value.copy(order=output)
        loadsize = value.nbytes
        if self.swapbyteorder and dtype.byteorder == '=':
            value.byteswap(True)
        if pad is not None:
            value = np.pad(value, pad, constant_values = padval)
        if output == 'list':
            return value.tolist()
        return value[()], loadsize

    # fundamental routine for 1D peeks with padding
    def peek_1d(self, length, dtype=np.float64, lead=0, tail=0, output = None, padval = None):
        """Read a 1D np array and pad on either side as specified.  Do not advance buffer pointer."""
        if not isinstance(dtype, np.dtype):
            dtype = np.dtype(dtype)
        ...
        if padval is not None:
            value[0:lead] = padval
            value[totlen-tail:totlen] = padval
        if output == 'list':
            return value.tolist()
        return value
    ...
    def peek_sn(self, *args, strip = False, offset = 0, order = 'F', output = 'C', codec = 'cp437', tail = None, **kwargs):
        ...
        call signature is peek_s([[dim,] length], ...)
    ...
    def get_n(self, *args, **kwargs):
        ...
        """
        value, loadsize = self.peek_n(*args, **kwargs)
        self.pos += loadsize
        return value
...
# helper functions to generate convenience functions
...
def _f_peek(p = None, **_kwargs):
    def _f(self, dim = (), offset = 0, **kwargs):
        return p(self, dim=dim, offset=offset, **kwargs).tolist()
    _f.p = p
    return _f

def _f_get_n(p = None, **_kwargs):
    def _f(self, dim = (), **kwargs):
        value, loadsize = p(self, dim=dim, **kwargs)
        self.pos += loadsize
        return value
    _f.p = p
    return _f
...
def _f_get(p = None, **_kwargs):
    def _f(self, dim = (), **kwargs):
        return p(self, dim=dim, **kwargs).tolist()
    _f.p = p
    return _f
...
def _f_peek_n1d(dt = None, **_kwargs):
...
def _f_peek_n1d_(dt = None, lead=0, tail=0, **_kwargs):
    def _f(self, length, **kwargs):
