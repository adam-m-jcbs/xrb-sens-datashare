...
class SolAbu(AbuSet):
    ...
    def path(self, name, sets, silent = False):
        ...
        if not os.path.isfile(filename):
            s = 'Data file for {} not found.'.format(filename)
            self.logger.critical(s)
            raise Exception(' [SolAbu] {}'.format(s))
        self.close_logger()
        return filename
...
def bbncoc(filename, write = False):
    ...
    while True:
        ...
        if lines[i].startswith(';'):
            offset +=2
        xions = [''.join(lines[i][k:k+10].split()) for k in range(offset, 70+offset, 10)]
        i += 1
...
class ScaledSolar(AbuModel):
    ...
    def __init__(self, scale = 1, Z = None, **kw):
        ...
        If Z is provided, overwrite scale by Z/Zsun.
