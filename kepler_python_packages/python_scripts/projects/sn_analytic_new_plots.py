...
#-------------------------------------------------------------------------------
    ...
    def luminosity(mdot, m, r_gain, time, zeta = 0.7, t_15 = 1.2, **kwargs):
        ...
        lum = (zeta * physconst.Kepler.gee * mdot * m / r_gain + l0 * np.exp(-time / t_cool)) #* alpha ** 3
        return lum

#-------------------------------------------------------------------------------

    def get_explosion(self, dump, offset = 0., silent = True, **kwargs):
        rn = dump.rn[1:-1]
        zm = dump.zm[1:-1]
        dn = dump.dnf[1:-1]
        st = dump.stot[1:-1]
        ma_conv = 0.0*dump.vconv[1:-1] ** 2 / (4.0/3.0 * dump.pn [1:-1] / dump.dn [1:-1])
        ybind = dump.ybind[1:-1]
        return self.criterion(dump.abu, rn, zm, dn, st, ybind, dump.core(), offset, silent, ma_conv = ma_conv, **kwargs)
#        return self.criterion(dump.abu, rn, zm, dn, st, ybind, dump.core(), offset,  silent, **kwargs)
...
#-------------------------------------------------------------------------------
    ...
    def criterion(self, abu, r, m, rho, stot, ybind, core, offset = 0,
        silent = False, threshold = 1, alpha_outflow = 0.5,
        eta_outflow = 1.5, beta = 7.0, alpha_turb = 1.37557, t_15=1.2,
        ma_conv = [0], addwind = True, **kwargs): """ What does this
        function do???
...
#        print(alpha_turb)
        ...
        lum0 = self.luminosity(macc, m, rg, time, t_15=t_15, **kwargs)
...
#        egain = (0.25 * physconst.Kepler.gee) * m / np.maximum(rsh, rg) + 7 * physconst.MEV / physconst.AMU * 0.75
        ...
        eta_acc = qheat / macc
#        psi = np.pi * ma_conv / 2.0 * (macc * m * physconst.Kepler.solmass ** 2/ rg) / qheat
#        scr = 4.0 / 3.0 * 0.4649
        tadv_theat = tadv / theat #* (1. + scr * (1+ psi) ** (2.0/3.0)) / (1. + scr)

        # ;; Compactness parameter
        if np.max(m/physconst.Kepler.solmass) > 2.5:
            xi25 = interp1d(m/physconst.Kepler.solmass,m/(r*1e-8*physconst.Kepler.solmass))(2.5).tolist()
        else:
            xi25 = 0.0
...
#        ebn_env = interp1d(m/physconst.Kepler.solmass,ybind)(core['iron core'].zm_sun).tolist()
        ebn_env = interp1d(m/physconst.Kepler.solmass,ybind)(mu4).tolist()
...
#            e_delayed = e_delayed + (m[i] - m[i-1]) * max([eta_outflow * eta_1 - eta_2,0.0]) * e_rec
            ...
            v_shock[i] = (con_matzner * np.sqrt(e_diag[i] / (m[i] - m_init)) * ((m[i] - m_init) / (rho[i] * r[i]**3))**0.19)

            m_by_rem = m_by_rem + (m[i] - m[i-1]) * (1.0-alpha_outflow)*(1.0-eta_1) #- m_out
...
#            if (((beta-1)/beta) * v_shock[i] < 0.8*v_esc[i]):
#                i_kick = i
...
#                e_wind = wind['e_wind']
        ...
        for i in range(i_final+1, jj[-1]):
            ...
            v_shock[i] = (con_matzner * np.sqrt(max([eexp,0.0]) / (m[i] - m_init)) * ((m[i] - m_init) / (rho[i] * r[i]**3))**0.19)
...
#                break
        ...
        if (m_init > m_max_by):
            m_init = 0.0
        #m_init = (-1.0 + np.sqrt (1.0 + 4.0 * 0.084 * m_init / physconst.Kepler.solmass)) / (2 * 0.084)  * physconst.Kepler.solmass
...
#-------------------------------------------------------------------------------
    ...
    def recalculate(self,  offset = 0, silent = False, threshold = 1, zeta = 0.7, alpha_outflow = 0.5, eta_outflow = 0.5, beta = 4, alpha_turb = 1.18, t_15=1.2, addwind = True, **kwargs):
        ...
        explosions = {}
        for dump in dumps:
            explosion = self.get_explosion(dump, offset = offset, threshold = threshold, zeta = zeta, alpha_outflow = alpha_outflow, beta = beta, alpha_turb = alpha_turb, t_15=t_15, addwind = addwind, silent = silent, **kwargs)
...
#-------------------------------------------------------------------------------
    ...
    def hist_kick(self, weight = 'imf', stacking = True, **kwargs):
...
#                l += r'\le{:d}'.format(limits[i])
        ...
        if stacking:
            plt.hist(xe_,bins=np.linspace(0,1200,13),
                     weights=imf_,normed=True,cumulative=False,stacked=True,
                     color = color, label = label, rwidth = 1.0
                     )
        else:
        ...
        #plt.title("")
        plt.xlabel(r"$v_\mathrm{kick}\ [\mathrm{km}\ \mathrm{s}^{-1}]$")
        plt.ylabel(r"$\mathrm{probability\ density} \ [10^{-51} \ \mathrm{erg}^{-1}]$")
        plt.legend(loc='best', prop={'size':12})
        plt.tight_layout()
        plt.show()
        plt.savefig('histogram_kick.pdf')



    def hist_frac(self, stacking = False, min_mass = 9.0, **kwargs):
...
#        limits = np.linspace (10,32,45*2)
        limits = np.linspace (min_mass-1e-3,32-1e-3,(32-min_mass)/0.01+1)
        m_ = [[] for i in range(len(limits))]
        xt_ = [[] for i in range(len(limits))]
        xe_ = [[] for i in range(len(limits))]
        fate_last = 1.0
        print(len(limits),len(m))
        print(limits)
        ...
        for i in range(len(xt_)):
            if (len(xt_[i]) > 0):
                ...
                fate_last = expl_frac[i]
            else:
                expl_frac [i] = fate_last
        limits = limits + 1e-3
...
#        limits = limits - 0.5 * (limits[1] - limits[0])
        ...
        plt.bar(limits,expl_frac,width=0.25)
#        plt.plot(limits,expl_frac)#,width=0.5)
        ...
        for i in range (len(xt_)):
            if i > 0:
                pcum [i] = pcum [i-1] + expl_frac [i-1] / limits [i-1] ** 2.3
            else:
                pcum [i] = 0.0
#                pcum [i] = expl_frac [i] / limits [i] ** 2.3
        ...
        jj = np.where(expl_frac > 0.9)
        print(jj)

#        plt.clf()
#        print(limits)
#        print('pcum',pcum)
#        data=np.zeros([len(jj),2])
#        data[:,0]=limits[jj]
#        data[:,1]=expl_frac[jj]
        jj = np.where(expl_frac > 0.9)
        np.savetxt('exploding_cases.dat',limits[jj],fmt='%.6f')
...
#        plt.savefig('cum_dist.pdf')
        ...
        jj = np.where(expl_frac > 0.9)
        lim_imf = np.linspace (0, 0.999999999, 2000)
        lim_imf = ((1.0 - lim_imf) / 9 ** 1.35) ** (-1.0/1.35)
        print(lim_imf)
        prb_imf = lim_imf * 0.0
        jj = np.where((lim_imf < 32) & (lim_imf > 9))
        print(min(limits))
        prb_imf[jj] = interp1d(limits,expl_frac)(lim_imf[jj]).tolist()
#        print(prb_imf)
        jj = np.where(prb_imf > 0.5)
        np.savetxt('exploding_cases_imf.dat',lim_imf[jj],fmt='%.6f')
    ...
    def hist (self, region = 'ns', stacking = True, **kwargs):
        ...
        xr_ = [[] for i in range(len(limits))]
        imf_ = [[] for i in range(len(limits))]
        mej_ = [[] for i in range(len(limits))]

        for _m, _xe, _xr, _imf, _mej in zip(m, xe, xr, imf, mej):
            for i,l in enumerate(limits):
                if (_m <= l):
                    m_[i] += [_m]
                    xe_[i] += [_xe]
...
#        imf=imf.tolist()
        ...
        print(mo)
        print(mf)
        ox_av = np.average (mo, weights = imf)
        fe_av = np.average (mf, weights = imf)
        mej_av = np.average (mej, weights = imf)
        print ('Average neutron star mass',mns_av)
        print ('Average [O/Fe]',np.log10(ox_av/fe_av/(1.57e7*8)*(7.78e5*56)))
        print ('O production factor',ox_av/mej_av/0.006851142699255012)
...
# Plot for paper
    ...
    def paper_plot(self,extra=False,poznanski=False,mode = 1, **kwargs):
        ...
        for iplot in iplots:
            ...
            if mode == -2:
                print("Don't recalculate explosion models.")
            xr = []
            xe = []
            m  = []
            xn = []
...
#        ax2.set_xlabel(r'$M_\mathrm{ZAMS} \  [M_\odot]$')
    ...
    def kick_plot(self,extra=False,poznanski=False,mode = 1, **kwargs):
...
#                m  += [mass]
                ...
                xe += [explosion['e_expl']]
                xr += [explosion['m_grav']]
                xkick += [explosion['v_kick']]

            xe = np.array(xe) * 1.e-51
            xr = np.array(xr) / physconst.Kepler.solmass
            xn = np.array(xn) / physconst.Kepler.solmass
            m  = np.array(m)
            xkick = np.array(xkick) / 1e5
            ...
            ax2.plot(m, xr, plotsyms [iplot], ms=1)
            ax3.plot(m, xr, plotsyms [iplot], ms=1)
            ax4.plot(m, xkick, plotsyms [iplot], ms=1)
        ...
        if mode == -2:
            ...
            b = 1.65
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*0+1.16,color='r')
            ax4.plot(mm,(mm-a)*1.3e3+180,color='r')

            a = b
            b = 2.4
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*0.3+1.32,color='r')
            ax4.plot(mm,(mm-a)*3.5e2+200,color='r')

            a = b
            b = 3.2
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*0.7+1.42,color='r')
            ax4.plot(mm,(mm-a)*1.1e3+400,color='r')

            a = b
            b = 3.6
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*0.25+1.32,color='r')
            ax4.plot(mm,(mm-a)*240+160,color='r')


            a = 4.05
            b = 4.6
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*0.0+1.5,color='r')
            ax4.plot(mm,(mm-a)*100+700,color='r')

            a = 5.7
            b = 6.0
            mm = a + xx * (b-a)
            ax3.plot(mm,(mm-a)*(-0.2)+1.64,color='r')
            ax4.plot(mm,(mm-a)*(-6e2)+550,color='r')
...
#        ax4.set_xlim([10,32.54])
        ...
        ax2.set_ylabel(r'$M_\mathrm{BH} \  [M_\odot]$')
        ax3.set_ylabel(r'$M_\mathrm{NS} \  [M_\odot]$')
        ax4.set_ylabel(r'$v_\mathrm{kick} \ [\mathrm{km} \, \mathrm{s}^{-1}]$')

#        ax1.set_xlabel(r'$M_\mathrm{ZAMS} \  [M_\odot]$')
#        ax2.set_xlabel(r'$M_\mathrm{ZAMS} \  [M_\odot]$')
        ax4.set_xlabel(r'$M_\mathrm{CO} \  [M_\odot]$')
        ...
        ax4.set_yticks (np.arange (0, 2000, 100), minor = True)

        ax4.set_xticks (np.arange (1, 10., 1))
        ax4.set_xticks (np.arange (1, 10., 0.2), minor = True)
        ...
        if mode == -2:
            f.savefig('kick_fit.pdf')
        if mode == -1:
            f.savefig('non_standard_kick.pdf')
        if mode == 0:
            f.savefig('standard_kick.pdf')
        if mode == 1:
            f.savefig('beta_kick.pdf')
        if mode == 2:
            f.savefig('alpha_turb_kick.pdf')
        if mode == 3:
            f.savefig('alpha_out_kick.pdf')
        if mode == 4:
            f.savefig('zeta_kick.pdf')
        if mode == 5:
            f.savefig('threshold_kick.pdf')
        if mode == 6 :
            f.savefig('t_cool_kick.pdf')
...
#-------------------------------------------------------------------------------
# Kick vs. explosion energy
    def vkick_vs_e(self,**kwargs):
        ...
        ax1.plot(xe, xk, 'k.', ms=2)


        ax1.set_ylabel(r'$E_\mathrm{expl} \ [10^{51}\, \mathrm{erg}]$')
        ax1.set_ylabel(r'$v_\mathrm{kick} \ [\mathrm{km}/\mathrm{s}]$')
        ax1.set_xscale('linear')
        ax1.set_yscale('linear')
        ax1.set_xlim([0, 2])
        ax1.set_ylim([0, 2000])
        ...
        f.savefig('vkick_vs_e.pdf')

#-------------------------------------------------------------------------------
# Kick vs. NS mass
    def vkick_vs_mgrav(self,**kwargs):
        ...
        ax1.plot(xm, xk, 'k.', ms=2)


        ax1.set_ylabel(r'$M_\mathrm{grav} \ [M_\odot]$')
        ax1.set_ylabel(r'$v_\mathrm{kick} \ [\mathrm{km}/\mathrm{s}]$')
        ax1.set_xscale('linear')
        ax1.set_yscale('linear')
        ax1.set_xlim([1, 2.1])
        ...
        f.savefig('vkick_vs_mgrav.pdf')



        #-------------------------------------------------------------------------------
# Helium core mass for Ken Nomoto

    def m_he_m_zams(self,**kwargs):
...
#        f.subplots_adjust (left=0.18, right=0.97, bottom=0.12, top = 0.95)
        ...
        jj1 = np.where(xe > 1e45)
        jj0 = np.where(xe <= 1e45)
        nc1 = plt.scatter(xm[jj1], xh[jj1], color='r',marker='.',label='exploding')
        nc2 = plt.scatter(xm[jj0], xh[jj0], color='k',marker='.',label='non-exploding')
        plt.legend((nc1,nc2),('exploding','non-exploding'),loc=2)

        plt.xlabel(r'$\mathrm{ZAMS\ mass} \ [M_\odot$]')
        plt.ylabel(r'$\mathrm{He\ core\ mass} \ [M_\odot]$')
        plt.xscale('linear')
        plt.yscale('linear')
        plt.xlim([9,32])
        plt.ylim([0, 15])
