...
class StarDB(AbuData, Logged):
    ...
    def _from_data(self, **kwargs):
        ...
        if self.fieldnames is None:
            self.fieldnames = np.array(
                self.fielddata.dtype.names,
                dtype = np.object)
        else:
            self.fieldnames = np.array(
                self.fieldnames,
        ...
        if self.fieldtypes is None:
            fieldtypes = []
            dtypes = {np.uint64:  self.Type.uint64,
        ...
        self.abundance_unit  = kwargs.get('abundance_unit'  , self.AbundanceUnit.mol_fraction)
        ...
        self.abundance_data  = kwargs.get('abundance_data'  , self.AbundanceData.all_ejecta)
        self.abundance_sum   = kwargs.get('abundance_sum'   , self.AbundanceSum.number_fraction)
    ...
    def _write_dbl(self, data):
        ...
        dim = data.shape
        value = np.ndarray(dim,
                           buffer = np.asfortranarray(data).data,
                           dtype = self.dtype_f8,
                           order = 'C').copy()
        if self.swapbyteorder:
             value.byteswap(True)
    ...
    def _load(self):
        ...
        if self.version < 10100:
            abu_A = np.zeros(nabu, dtype=np.uint64)
            abu_E = np.zeros(nabu, dtype=np.uint64)
        else:
            abu_A = self._read_uin(self.nabu)
            abu_E = self._read_uin(self.nabu)

        if self.abundance_type == 0:
            self.ions = np.array([isotope.ion(Z=int(abu_Z[i]),
                                              A=int(abu_A[i]),
                                              E=int(abu_E[i]))
            ...
            self.ions = np.array([isotope.ion(A=int(abu_A[i]))
                                  for i in range(self.nabu)])
        elif self.abundance_type == 4:
            self.ions = np.array([isotope.ion(N=int(abu_A[i]))
                                  for i in range(self.nabu)])
        else:
        ...
        # load field data
        self.fielddata = self._read_stu(self.nstar,
                                         self.fieldnames,
                                         self.fieldtypes)
    ...
    def print_info(self):
        ...
        for ifield in range(self.nfield):
            ...
            if len(np.argwhere(self.type_names[self.fieldtypes[ifield]]
    ...
    def _compute_index(self, array_limit = 2**24):
        ...
        for ifield in range(self.nfield):
            vindex[ifield] = scipy.sparse.lil_matrix((self.nstar, self.nstar), dtype=np.uint64)
        ...
        if self.nfield * nvmax * nimax <= array_limit:
            ...
            for ifield in range(self.nfield):
                v[ifield, :self.nvalues[ifield], :imax[ifield]] = vindex[ifield].toarray()
            vindex = v
        else:
    ...
    def get_star_slice(self, *args, **kwargs):
        ...
        """
        if len(args) == 1 and isinstance(agrs[0], collections.Mapping):
            fields = args[0]
        else:
            fields = kwargs
        for key in fields:
    ...
    def index_info(self, indices):
        ...
        print('-'*maxlen)
        for i in indices:
            print('{name:>{maxlenname}s}: {index:>{maxlenvalue}d}'.format(
                name = 'index',
                index = i,
                maxlenvalue = maxlenvalue,
                maxlenname = maxlenname,
                ))
            for f,n,u in zip(self.fieldformats,
                             self.fieldnames,
                             self.fieldunits):
                print('{name:>{maxlenname:d}s}: {value:>{maxlenvalue}s} {unit:s}'.format(
                    name = n,
                    unit = u,
                    value = '{value:{format:s}}'.format(
                        value = self.fielddata[i][n],
                        format = f),
                    maxlenname = maxlenname,
                    maxlenvalue = maxlenvalue,
                    ))
    ...
    def abundance_info(self, indices):
        ...
        print(('{:>8s}' + ' '.join(['{:>12d}'] * len(indices))).format(
            'Ion', *indices))
        print('=' * (9 +  13 * len(indices)))
        for ion, abu in zip(self.ions, self.data[indices,:].transpose()):
            print(('{!s:>8}' + ' '.join(['{:12.5e}'] * len(indices))).format(
                ion,
                *abu
                ))
        print('=' * (9 +  13 * len(indices)))
