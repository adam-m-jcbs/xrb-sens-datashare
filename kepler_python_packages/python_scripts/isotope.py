...
class Ion(object, metaclass = MetaSingletonHash):
    ...
    def LaTeX(self, **kwargs):
        ...
        if self.is_lepton():
            f = (r'\mathrm{e}', r'\mu', r'\tau')[self.E // 2]
            if self.Z == 0:
                if self.E % 2 == 1:
                    l = r'\nu'
                else:
    ...
    _kobayashi = re.compile('\^(\d+)\^([A-Za-z])')
    ...
    def _decompose(cls,
        ...
        if len(c) == 1 and len(c[0]) > 1:
            ...
            if cx[0] in iso_strings_:
                if not cx in elements and cx[1:] in elements_:
                    c = [c[0][0], c[0][1:]]
        ...
        if len(c) == 2 and c == ['(', ')']:
            if len(n) == 1:
                ...
                return (s,) + ('',)*3
        # experimental
        if el in iso_special_mass:
                n.insert(0, str(iso_special_mass[el]))
        ...
        if len(c) == 2:
            if c[1] in iso_strings_ground:
                ...
                if e == '1':
                    e = str(cls.EANY)
            if not c[1] in iso_strings and not c[1][0] == '*':
                return (s,) + ('',)*3
...
#        if len(el) == 2 and el(2)
    ...
    def _add(self, x, sign1 = +1, sign2 = +1):
        ...
        if not (((self.F & self.F_GROUP_MASK == self.F_ISOMER) and x.is_photon()) or
            ...
            E = None
        if ((self.F & self.F_GROUP_MASK == self.F_ELEMENT) and
...
ufunc_ion_kw  = lambda y, kw: np.array(np.frompyfunc(lambda x: ion(x, **kw), 1, 1)(y), dtype = np.object)
...
class IonFactory():
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) == 1 and len(kwargs) == 0 and isinstance(args[0], Ion):
            return args[0]
        if len(args) == 1 and (isinstance(args[0], np.ndarray) or
                               isinstance(args[0], list)):
            return ufunc_ion_kw(args[0], kwargs)
        kwargs = kwargs.copy()
