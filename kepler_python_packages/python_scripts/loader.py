...
class Loader(Logged):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            try:
                ...
                if not kwargs.get('silent', silent):
                    self.logger.error(' [{:s}] ERROR: File "{:s}" not found.'.format(
                        self.name,
                        filename))
        # Maybe this should be done as part of the FortranReader class...
        # but it is ...
        ...
        if len(files) != 1:
            ...
            if len(args) > 0:
                ...
                if os.path.isdir(p):
                    ...
                    fd_args['initialdir'] = os.path.dirname(p)
            if self.extension != '*':
                fd_args['filetypes'] = [(self.name + ' files', '.' + self.extension + comp) for comp in compression_types]
        ...
        try:
            ...
            self.logger.error(' [{:s}] ERROR Loading File "{:s}".'.format(
                    self.name,
                    filename))
            return None
