...
def getfallback(link, dump):
    ...
    out_file = base + '.out'
    with open(cmd_file,"w") as fcmd:
        fcmd.write("z xbind rn un uesc,kill\n")
    os.system("rm -f "+ out_file)
    call = kepler + " " + base + " " + base_name + link
    if (dump == 0):
        call += "#envel"
    else:
        call += "#final"
    call += " s k <" + cmd_file + " >" + out_file
    print("CALL: " + call)
    os.system(call)
    zone=0
    mass=0.    
        ...
        for line in fout:
            if not start:
                ...
                if len(items) != 6:
                    fail = True
                    break
                uesc = float(items.pop())
                un   = float(items.pop())
                rn   = float(items.pop())
                xbind= float(items.pop())
                mass = float(items.pop())
                zone = int  (items.pop())

                if ((xbind > 0.) and (rn > 1.e10) and (un > 0.01 * uesc)):
                    break                
...
def getresult(link, zone):
    ...
    out_file = base + '.out'
    with open(cmd_file,"w") as fcmd:
        fcmd.write("e,sumisun {:4d} 9999 ni56,kill\n".format(zone))
    os.system("rm -f "+ out_file)
    call = kepler + " " + base + " " + base_name + link
    call += "#final s k < " + cmd_file + " >" + out_file
    print("CALL: " + call)
    os.system(call)
    ekin=0.
    mni=0.
        ...
        for line in fout:
            if part == 0:
                if line.startswith(" ke ="):
                    ekin = float(line[5:17])
                    part += 1
            elif part == 1:
                if line.startswith("ni56 "):
                    mni = float(line[5:16])
                    part += 1
            else:
                break
...
""".format(alpha_string,
        ...
        for line in fin:
            fout.write(line)
    os.system("mv -f {} {}".format(temp_link_name,link_file_name))
